...
def database_status(_: str = Depends(require_admin)):
    ...
    return {
        ...
        "counts": counts,
        "scopes": {
            **{k: v["label"] for k, v in RESET_SCOPES.items()},
            "operational": "All Operational Data",
...
def database_reset(payload: ResetPayload, _: str = Depends(require_admin)):
    ...
    try:
        result = reset_database(
            payload.scope, create_backup=True if payload.scope == "factory" else payload.create_backup
        )
