(() => {
  ...
  function install() {
    ...
    section.innerHTML = `
      <div id="sjListView">
        ...
        <div class="table-wrap sj-run-table"><table><thead><tr><th>Job</th><th>Status</th><th>Fetched</th><th>Matches</th><th>Notifications</th><th>Started</th></tr></thead><tbody id="sjRuns"></tbody></table></div>
      </div>
  ...
  window.loadSearchJobRuns = async function() {
    ...
    const body = $('sjRuns');
    if (!body) return;
    body.innerHTML = (data.runs || []).slice(0, 20).map(run => `<tr><td>${esc(run.search_job_name)}</td><td>${esc(run.status)}</td><td>${run.fetched}</td><td>${run.matches}</td><td>${esc((run.notification_channels || []).join(', ') || '—')}</td><td>${esc(formatDate(run.started_at))}</td></tr>`).join('') || '<tr><td colspan="6" class="muted">No job runs yet.</td></tr>';
  };
  ...
  window.runSearchJob = async function(id) {
    ...
    try { const result = await api(`/api/search-jobs/${id}/run`, {method: 'POST'}); toast(`${result.matches} new matches found`); await loadSearchJobs(); } catch (error) { toast(error.message, true); }
  };
