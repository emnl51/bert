...
def list_candidates():
    ...
    with connection() as con:
        rows = con.execute("SELECT * FROM candidate_profiles ORDER BY name").fetchall()
...
def get_candidate(candidate_id: int):
    ensure_candidate_schema()
    with connection() as con:
        row = con.execute("SELECT * FROM candidate_profiles WHERE id=?", (candidate_id,)).fetchone()
...
def save_candidate(data: dict[str, Any], candidate_id: int | None = None):
    ...
    with connection() as con:
        if candidate_id:
            con.execute(
                "UPDATE candidate_profiles SET name=:name,headline=:headline,cv_text=:cv_text,skills_json=:skills_json,languages_json=:languages_json,target_roles_json=:target_roles_json,notes=:notes,updated_at=:updated_at WHERE id=:id",
                {**vals, "id": candidate_id},
            )
            return candidate_id
        cur = con.execute(
            "INSERT INTO candidate_profiles(name,headline,cv_text,skills_json,languages_json,target_roles_json,notes,created_at,updated_at) VALUES(:name,:headline,:cv_text,:skills_json,:languages_json,:target_roles_json,:notes,:created_at,:updated_at)",
            {**vals, "created_at": now},
        )
...
def delete_candidate(candidate_id: int):
    ...
    with connection() as con:
        used = con.execute(
            "SELECT COUNT(*) FROM search_job_candidates WHERE candidate_profile_id=?", (candidate_id,)
        ).fetchone()[0]
        if used:
...
def candidate_for_search_job(search_job_id: int):
    ...
    with connection() as con:
        row = con.execute(
            """SELECT c.* FROM search_job_candidates m JOIN candidate_profiles c ON c.id=m.candidate_profile_id WHERE m.search_job_id=? AND m.enabled=1""",
            (search_job_id,),
        ).fetchone()
    return _decode(row) if row else None
...
def mapping_for_jobs():
    ensure_candidate_schema()
    with connection() as con:
        rows = con.execute("SELECT search_job_id,candidate_profile_id,enabled FROM search_job_candidates").fetchall()
