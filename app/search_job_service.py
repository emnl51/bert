...
async def run_search_job(search_job_id: int) -> dict:
    ...
    try:
        ...
        return {'search_job_id':search_job_id,'name':search_job['name'],'profile':profile['name'],'fetched':len(fetched),'matches':len(matches),'provider_errors':provider_errors,'notification_channels':channels}
