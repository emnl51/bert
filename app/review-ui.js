(() => {
  ...
  function injectStyles(){
    const st=document.createElement('style'); st.textContent=`
      ...
      .job-meta{font-size:12px;color:var(--muted);display:flex;gap:10px;flex-wrap:wrap}.why-list{font-size:12px;color:var(--muted);max-height:52px;overflow:hidden}.learning-rule{display:grid;grid-template-columns:90px 90px 1fr 80px 110px auto;gap:10px;align-items:center;padding:10px 12px;border-bottom:1px solid var(--line)}
      ...
      .polarity-boost{background:var(--goodbg);color:var(--good)}.polarity-penalty{background:var(--badbg);color:var(--bad)}
  ...
  function installOverview(){
    ...
    wrap.innerHTML=`<div class="section-title"><div><h2>Job Review Queue</h2><div class="hint">Review jobs, track applications and teach JobTrack what fits — and what does not.</div></div></div>
  ...
  function installLearning(){
    ...
    const s=document.createElement('section');s.id='learning';s.className='section';s.innerHTML=`
      <div class="section-title"><div><h2>Search Learning</h2><div class="hint">Boost rules come from Suitable / Applied / Interview / Offer. Penalty rules come from Not suitable feedback. All rules are reversible.</div></div><button class="btn" onclick="loadLearning()">Refresh</button></div>
      <div class="grid">
        <div class="card"><div class="muted">Positive events</div><div id="lfPositive" class="metric">—</div></div>
        <div class="card"><div class="muted">Active boost rules</div><div id="lfBoosts" class="metric">—</div></div>
        <div class="card"><div class="muted">Not suitable</div><div id="lfBad" class="metric">—</div></div>
        <div class="card"><div class="muted">Active penalty rules</div><div id="lfPenalties" class="metric">—</div></div>
      </div>
  ...
  async function loadReviewJobs(){
    ...
    grid.innerHTML=d.jobs.length?d.jobs.map((j,i)=>`<article class="job-card">
      ...
      <div class="why-list">${(j.reasons||[]).slice(0,8).map(x=>`<span class="pill">${esc(x)}</span>`).join('')}</div>
  ...
  window.reviewJob=async function(jobKey,suitability,reason='',note='',learn=true){
    await api(`/api/jobs/${encodeURIComponent(jobKey)}/feedback`,{method:'POST',body:JSON.stringify({suitability,reason,note,learn})});
    toast(suitability==='suitable'?'Added to To Apply and learned as a positive preference':'Feedback saved');
    await loadReviewJobs(); if(window.loadOverview)loadOverview();
  }
  ...
  window.loadLearning=async function(){
    ...
    $('learnedRules').innerHTML=d.rules.length?d.rules.map(r=>`<div class="learning-rule">
      <span class="pill ${r.polarity==='boost'?'polarity-boost':'polarity-penalty'}">${r.polarity==='boost'?'BOOST':'PENALTY'}</span>
      <span class="pill">${esc(r.scope)}</span>
      <span class="rule-term">${esc(r.term)}</span>
      <span>${r.weight>0?'+':''}${r.weight}</span>
      <span>${r.evidence_count} signals${r.strongest_event?` · ${esc(r.strongest_event)}`:''}</span>
      <span><button class="btn small" onclick="toggleRule(${r.id},${!r.enabled})">${r.enabled?'Disable':'Enable'}</button> <button class="btn small danger" onclick="removeRule(${r.id})">Delete</button></span>
    </div>`).join(''):'<div class="muted" style="padding:16px">No learned rules yet.</div>';
