...
def _decode(row, mask_secrets: bool) -> dict[str, Any]:
    d = dict(row)
    for k in ('enabled','notify_telegram','notify_email'): d[k] = bool(d[k])
...
def list_search_jobs(mask_secrets: bool = True) -> list[dict[str, Any]]:
    ...
    with connection() as con:
        rows = con.execute('''SELECT sj.*,sp.name AS profile_name FROM search_jobs sj JOIN search_profiles sp ON sp.id=sj.profile_id ORDER BY sj.enabled DESC,sj.id''').fetchall()
...
def get_search_job(job_id: int, mask_secrets: bool = False) -> dict[str, Any] | None:
    ensure_search_job_schema()
    with connection() as con:
        row = con.execute('''SELECT sj.*,sp.name AS profile_name FROM search_jobs sj JOIN search_profiles sp ON sp.id=sj.profile_id WHERE sj.id=?''',(job_id,)).fetchone()
    return _decode(row,mask_secrets) if row else None


def save_search_job(data: dict[str, Any], job_id: int | None = None) -> int:
    ensure_search_job_schema(); now=_now()
    ...
    with connection() as con:
        ...
        if job_id:
            ...
            if not row: raise ValueError('Search job not found')
            existing_secrets=json.loads(row['secrets_json'] or '{}')
        for key,value in supplied_secrets.items():
            if key in SECRET_FIELDS and value and value != 'configured': existing_secrets[key]=encrypt_secret(value)
        vals={
            'name':str(data.get('name','Search Job')).strip(),'enabled':int(bool(data.get('enabled',True))),
            'profile_id':int(data['profile_id']),'target_location':str(data.get('target_location','Berlin')),
            'location_terms_json':json.dumps(data.get('location_terms',[]),ensure_ascii=False),'source_ids_json':json.dumps(data.get('source_ids',[])),
            'frequency':data.get('frequency','weekly'),'day_of_week':data.get('day_of_week','mon'),'hour':int(data.get('hour',8)),
            'minute':int(data.get('minute',0)),'interval_hours':int(data.get('interval_hours',12)),
            'min_score_override':data.get('min_score_override'),'min_language_score_override':data.get('min_language_score_override'),
            'max_results':int(data.get('max_results',20)),'notify_telegram':int(bool(data.get('notify_telegram',False))),
            'notify_email':int(bool(data.get('notify_email',False))),'notification_json':json.dumps(notification,ensure_ascii=False),
            'secrets_json':json.dumps(existing_secrets),'updated_at':now,
        }
        if job_id:
            con.execute('''UPDATE search_jobs SET name=:name,enabled=:enabled,profile_id=:profile_id,target_location=:target_location,location_terms_json=:location_terms_json,source_ids_json=:source_ids_json,frequency=:frequency,day_of_week=:day_of_week,hour=:hour,minute=:minute,interval_hours=:interval_hours,min_score_override=:min_score_override,min_language_score_override=:min_language_score_override,max_results=:max_results,notify_telegram=:notify_telegram,notify_email=:notify_email,notification_json=:notification_json,secrets_json=:secrets_json,updated_at=:updated_at WHERE id=:id''',{**vals,'id':job_id})
            return job_id
        cur=con.execute('''INSERT INTO search_jobs(name,enabled,profile_id,target_location,location_terms_json,source_ids_json,frequency,day_of_week,hour,minute,interval_hours,min_score_override,min_language_score_override,max_results,notify_telegram,notify_email,notification_json,secrets_json,created_at,updated_at)
                           VALUES(:name,:enabled,:profile_id,:target_location,:location_terms_json,:source_ids_json,:frequency,:day_of_week,:hour,:minute,:interval_hours,:min_score_override,:min_language_score_override,:max_results,:notify_telegram,:notify_email,:notification_json,:secrets_json,:created_at,:updated_at)''',{**vals,'created_at':now})
...
def create_search_job_run(job_id: int) -> int:
    with connection() as con:
        return int(con.execute("INSERT INTO search_job_runs(search_job_id,started_at,status) VALUES(?,?,'running')",(job_id,_now())).lastrowid)


def finish_search_job_run(run_id: int, job_id: int, status: str, fetched: int=0, matches: int=0, provider_errors=None, channels=None, error: str='') -> None:
    now=_now()
    with connection() as con:
        con.execute('''UPDATE search_job_runs SET finished_at=?,status=?,fetched=?,matches=?,provider_errors_json=?,notification_channels_json=?,error=? WHERE id=?''',(now,status,fetched,matches,json.dumps(provider_errors or []),json.dumps(channels or []),error,run_id))
        con.execute('UPDATE search_jobs SET last_run_at=?,last_run_status=?,last_match_count=?,updated_at=? WHERE id=?',(now,status,matches,now,job_id))


def list_search_job_runs(job_id: int | None=None, limit: int=100) -> list[dict[str, Any]]:
    ensure_search_job_schema(); params=[]; where=''
    if job_id: where='WHERE r.search_job_id=?'; params.append(job_id)
    params.append(limit)
    with connection() as con:
        rows=con.execute(f'''SELECT r.*,sj.name AS search_job_name FROM search_job_runs r JOIN search_jobs sj ON sj.id=r.search_job_id {where} ORDER BY r.id DESC LIMIT ?''',params).fetchall()
    out=[]
    for r in rows:
        d=dict(r); d['provider_errors']=json.loads(d.pop('provider_errors_json') or '[]'); d['notification_channels']=json.loads(d.pop('notification_channels_json') or '[]'); out.append(d)
    return out
