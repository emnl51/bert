...
WERKSTUDENT_KEYWORDS = {
    'search': {
        'werkstudent supply chain': 0, 'working student supply chain': 0,
        'werkstudent procurement': 0, 'working student procurement': 0,
        'werkstudent supply planning': 0, 'working student supply planning': 0,
        'werkstudent order management': 0, 'werkstudent operations': 0,
        'werkstudent logistik': 0, 'working student logistics': 0,
    },
    'title': {
        'supply chain': 32, 'supply planning': 32, 'procurement': 30, 'einkauf': 30,
        'order management': 28, 'operations': 22, 'logistics': 24, 'material planning': 28,
    },
    'format': {
        'werkstudent': 34, 'working student': 34, 'student assistant': 26,
        'studentische aushilfe': 26, 'teilzeit': 14, 'part-time': 14, 'part time': 14,
    },
    'skill': {
        'sap': 7, 's/4hana': 7, 'power bi': 6, 'excel': 4, 'supplier': 5,
        'erp': 5, 'production planning': 7, 'international': 4, 'customer': 4,
        'rfq': 5, 'sourcing': 5, 'inventory': 5,
    },
    'negative': {
        'software engineer': -35, 'developer': -30, 'nurse': -50, 'pflege': -50,
        'driver': -35, 'fahrer': -35, 'restaurant': -35, 'senior director': -20,
    },
}

FULLTIME_KEYWORDS = {
    'search': {
        'supply chain manager berlin': 0, 'supply chain operations manager berlin': 0,
        'customer supply chain manager berlin': 0, 'operations manager berlin': 0,
        'procurement manager berlin': 0, 'strategic sourcing manager berlin': 0,
        'senior supply chain specialist berlin': 0,
    },
    'title': {
        'supply chain manager': 38, 'supply chain operations': 36, 'customer supply chain': 36,
        'operations manager': 32, 'procurement manager': 34, 'strategic sourcing': 32,
        'senior supply chain': 32, 'supply planning manager': 32, 'logistics manager': 26,
    },
    'format': {'full-time': 16, 'full time': 16, 'vollzeit': 16, 'permanent': 10},
    'skill': {
        'sap': 9, 's/4hana': 9, 'power bi': 7, 'erp': 6, 'supplier management': 8,
        'strategic sourcing': 8, 'procurement': 8, 'production planning': 7,
        'inventory': 6, 'international': 6, 'customer': 5, 'leadership': 6,
        'stakeholder': 6, 'incoterms': 5, 'rfq': 5,
    },
    'negative': {
        'software engineer': -40, 'developer': -35, 'nurse': -50, 'pflege': -50,
        'driver': -35, 'fahrer': -35, 'restaurant': -35, 'internship': -18, 'praktikum': -18,
    },
}
...
def ensure_profile_schema() -> None:
    ...
    with connection() as con:
        ...
        if con.execute('SELECT COUNT(*) FROM search_profiles').fetchone()[0] == 0:
            common_locations = json.dumps(['berlin','potsdam','hennigsdorf','ludwigsfelde','teltow','wildau','schönefeld','schoenefeld','oranienburg','brandenburg'])
            con.execute('''INSERT INTO search_profiles(name,slug,enabled,is_default,target_location,location_terms_json,min_score,min_language_score,language_weight,current_german_level,max_german_requirement,show_b2_stretch,hide_german_heavy,prefer_german_growth,keywords_json,created_at,updated_at)
                           VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                        ('Werkstudent / Part-time','werkstudent',1,1,'Berlin',common_locations,35,40,35,'a2_b1','b1',1,1,1,json.dumps(WERKSTUDENT_KEYWORDS),now,now))
            con.execute('''INSERT INTO search_profiles(name,slug,enabled,is_default,target_location,location_terms_json,min_score,min_language_score,language_weight,current_german_level,max_german_requirement,show_b2_stretch,hide_german_heavy,prefer_german_growth,keywords_json,created_at,updated_at)
                           VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                        ('Full-time Supply Chain','fulltime',1,0,'Berlin',common_locations,40,35,25,'b1','b2',1,0,1,json.dumps(FULLTIME_KEYWORDS),now,now))


def _row_to_profile(row) -> dict[str, Any]:
    p = dict(row)
    p['enabled'] = bool(p['enabled']); p['is_default'] = bool(p['is_default'])
    p['show_b2_stretch'] = bool(p['show_b2_stretch']); p['hide_german_heavy'] = bool(p['hide_german_heavy'])
...
def save_profile(data: dict[str, Any], profile_id: int | None = None) -> int:
    ensure_profile_schema(); now=_now()
    fields = {
        'name': data.get('name','Search Profile').strip(), 'slug': data.get('slug','profile').strip().lower().replace(' ','-'),
        'enabled': int(bool(data.get('enabled',True))), 'is_default': int(bool(data.get('is_default',False))),
        'target_location': data.get('target_location','Berlin'),
        'location_terms_json': json.dumps(data.get('location_terms',[]), ensure_ascii=False),
        'min_score': int(data.get('min_score',35)), 'min_language_score': int(data.get('min_language_score',40)),
        'language_weight': int(data.get('language_weight',35)), 'current_german_level': data.get('current_german_level','a2_b1'),
        'max_german_requirement': data.get('max_german_requirement','b1'),
        'show_b2_stretch': int(bool(data.get('show_b2_stretch',True))), 'hide_german_heavy': int(bool(data.get('hide_german_heavy',True))),
    ...
    with connection() as con:
        ...
        if profile_id:
            con.execute('''UPDATE search_profiles SET name=:name,slug=:slug,enabled=:enabled,is_default=:is_default,target_location=:target_location,location_terms_json=:location_terms_json,min_score=:min_score,min_language_score=:min_language_score,language_weight=:language_weight,current_german_level=:current_german_level,max_german_requirement=:max_german_requirement,show_b2_stretch=:show_b2_stretch,hide_german_heavy=:hide_german_heavy,prefer_german_growth=:prefer_german_growth,keywords_json=:keywords_json,updated_at=:updated_at WHERE id=:id''',
                        {**fields,'updated_at':now,'id':profile_id})
            return profile_id
        cur=con.execute('''INSERT INTO search_profiles(name,slug,enabled,is_default,target_location,location_terms_json,min_score,min_language_score,language_weight,current_german_level,max_german_requirement,show_b2_stretch,hide_german_heavy,prefer_german_growth,keywords_json,created_at,updated_at)
...
def delete_profile(profile_id: int) -> None:
    ...
    with connection() as con:
        ...
        if not row: return
        if row['is_default']: raise ValueError('Default profile cannot be deleted')
        con.execute('DELETE FROM search_profiles WHERE id=?',(profile_id,))


def upsert_profile_score(job, profile_id: int) -> None:
    with connection() as con:
        con.execute('''INSERT INTO job_profile_scores(job_key,profile_id,job_score,language_score,overall_score,language_label,reasons_json,language_reasons_json,updated_at)
                    ...
                    (job.key,profile_id,job.score,job.language_score,job.overall_score,job.language_label,json.dumps(job.reasons,ensure_ascii=False),json.dumps(job.language_reasons,ensure_ascii=False),_now()))
