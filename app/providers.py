...
async def fetch_adzuna(source: dict, search_terms: list[str], target_location: str) -> list[Job]:
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
        for term in search_terms:
            ...
            response = await client.get(
                "https://api.adzuna.com/v1/api/jobs/de/search/1?" + urlencode(params),
...
async def fetch_rss(source: dict, search_terms: list[str], target_location: str) -> list[Job]:
    ...
    for entry in parsed.entries[:200]:
        ...
        location = entry.get("location", "") or config.get("default_location", "")
...
async def fetch_greenhouse(source: dict, search_terms: list[str], target_location: str) -> list[Job]:
    ...
    return [
        Job(
            source=source["name"],
            external_id=str(i.get("id") or _stable_id(i.get("title", ""), i.get("absolute_url", ""))),
            title=i.get("title") or "",
            company=source.get("config", {}).get("company_name", token),
            location=(i.get("location") or {}).get("name", ""),
            url=i.get("absolute_url") or "",
            description=i.get("content") or "",
            created_at=i.get("updated_at") or "",
            remote="remote" in ((i.get("location") or {}).get("name", "").lower()),
        )
        for i in response.json().get("jobs", [])
    ]
...
async def fetch_lever(source: dict, search_terms: list[str], target_location: str) -> list[Job]:
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
        response = await client.get(
            f"https://api.lever.co/v0/postings/{site}", params={"mode": "json"}, headers={"User-Agent": "JobTrack/5.0"}
        )
        response.raise_for_status()
    jobs = []
    for item in response.json():
        ...
        jobs.append(
            Job(
                source=source["name"],
                external_id=str(item.get("id") or _stable_id(item.get("text", ""), item.get("hostedUrl", ""))),
                title=item.get("text") or "",
                company=source.get("config", {}).get("company_name", site),
                location=loc,
...
async def fetch_smartrecruiters(source: dict, search_terms: list[str], target_location: str) -> list[Job]:
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
        while offset < 300:
            response = await client.get(
                base, params={"limit": 100, "offset": offset}, headers={"User-Agent": "JobTrack/5.0"}
            )
            response.raise_for_status()
            data = response.json()
            ...
            for item in content:
                ...
                jobs.append(
                    Job(
                        source=source["name"],
                        external_id=str(
                            item.get("id") or item.get("uuid") or _stable_id(item.get("name", ""), job_url)
                        ),
                        title=item.get("name") or item.get("title") or "",
                        company=source.get("config", {}).get("company_name", company),
                        location=loc,
                        url=job_url,
                        description=description,
                        created_at=item.get("releasedDate") or "",
                        remote="remote" in loc.lower(),
                    )
                )
            offset += len(content)
...
def _source_key(source: dict) -> str:
    return str(source.get("id") or f"{source.get('source_type')}:{source.get('name')}")
...
async def _fetch_with_resilience(provider, source: dict, search_terms: list[str], target_location: str):
    key = _source_key(source)
    failures, open_until = _CIRCUITS.get(key, (0, 0.0))
    if open_until > time.monotonic():
        raise RuntimeError("provider circuit is temporarily open")
    timeout = max(10, min(int(source.get("config", {}).get("total_timeout_seconds", 180)), 600))
    for attempt in range(3):
        try:
            jobs = await asyncio.wait_for(provider(source, search_terms, target_location), timeout=timeout)
            ...
            if not _is_transient(exc) or attempt == 2:
                if failures >= 3:
                    _CIRCUITS[key] = (failures, time.monotonic() + 300)
                raise
...
async def test_source(source: dict, search_terms: list[str], target_location: str) -> dict:
    ...
    if source["source_type"] == "search_link":
        ...
        return {
            ...
            "count": 0,
            "mode": "search-only",
            "search_url": render_search_url(template, query, target_location) if template else "",
        }
    try:
        jobs = await provider(test_copy, search_terms[:2], target_location)
    except Exception as exc:
        raise RuntimeError(_safe_provider_error(source, exc)) from None
    return {
        "ok": True,
        "count": len(jobs),
        "sample": [{"title": j.title, "company": j.company, "location": j.location} for j in jobs[:3]],
    }
