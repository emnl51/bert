...
STOP = {
    'the','and','with','for','from','your','you','our','are','will','this','that','und','der','die','das','mit','für','von',
    'eine','einer','einem','ein','im','in','auf','zu','als','bei','working','student','werkstudent','manager','senior','junior',
    'job','role','team','company','candidate','experience','years','year','skills','knowledge','responsible','responsibility',
}
...
TERM_GROUPS = {
    'technical': {
        ...
        'process optimization': ('process optimization', 'process optimisation', 'continuous improvement', 'process improvement'),
...
RESPONSIBILITY_GROUPS = {
    'cross-functional collaboration': ('cross-functional', 'cross functional', 'stakeholder', 'coordinate with', 'collaboration'),
    'process improvement': ('process improvement', 'process optimization', 'continuous improvement', 'optimize processes'),
...
def _role_requirement(candidate: dict, job: dict) -> tuple[dict, dict]:
    ...
    evidence = {
        'requirement_id': requirement['id'], 'category': 'role', 'term': job.get('title', ''), 'status': status,
        'evidence': best_target or 'No matching target role/headline evidence', 'confidence': round(value, 2),
    }
...
def _extract_term_requirements(job_text: str) -> list[dict]:
    ...
    for category, groups in TERM_GROUPS.items():
        for canonical, aliases in groups.items():
            ...
            if hit:
                requirements.append({'id': f'{category}:{canonical}', 'category': category, 'term': canonical, 'required': True, 'job_evidence': _excerpt(job_text, hit)})
    for canonical, aliases in RESPONSIBILITY_GROUPS.items():
        hit = _has(job_text, aliases)
        if hit:
            requirements.append({'id': f'responsibilities:{canonical}', 'category': 'responsibilities', 'term': canonical, 'required': True, 'job_evidence': _excerpt(job_text, hit)})
    return requirements
...
def _term_evidence(requirement: dict, candidate_text: str) -> dict:
    ...
    if overlap >= 0.5 and alias_tokens:
        return {
            'requirement_id': requirement['id'], 'category': category, 'term': term, 'status': 'partial',
            'evidence': f'Partial terminology overlap: {", ".join(sorted(candidate_tokens & alias_tokens))}', 'confidence': 0.5,
        }
    return {
        'requirement_id': requirement['id'], 'category': category, 'term': term, 'status': 'missing',
        'evidence': 'No supporting CV evidence found', 'confidence': 0.0,
    }
...
def _experience_evidence(job_text: str, candidate_text: str) -> tuple[list[dict], list[dict]]:
    ...
    requirement = {'id': 'experience:years', 'category': 'experience', 'term': f'{required}+ years relevant experience', 'required': True}
    ...
    return [requirement], [{'requirement_id': requirement['id'], 'category': 'experience', 'term': requirement['term'], 'status': status, 'evidence': text, 'confidence': confidence}]
...
def _deterministic(candidate: dict, job: dict) -> dict[str, Any]:
    ...
    if any(x in normalized_job for x in ('c1 german','c2 german','fluent german','business fluent german','verhandlungssicheres deutsch','deutsch c1')):
...
def _decode_row(row) -> dict:
    d = dict(row)
    for key in ('strengths','gaps','risks','matched_terms','missing_terms','requirements','evidence','transferable'):
...
def analyze_job(job_key: str, candidate_profile_id: int, search_job_id: int | None = None, force: bool = False) -> dict[str, Any]:
...
def get_analysis(job_key: str, candidate_profile_id: int):
    ensure_intelligence_schema()
    with connection() as con:
        row = con.execute('SELECT * FROM job_intelligence WHERE job_key=? AND candidate_profile_id=?', (job_key, candidate_profile_id)).fetchone()
...
def list_analyses(candidate_profile_id: int | None = None, limit: int = 300):
    ...
    params.append(limit)
    with connection() as con:
        rows = con.execute(f'''SELECT i.*,j.title,j.company,j.location,j.url FROM job_intelligence i JOIN jobs j ON j.job_key=i.job_key {where} ORDER BY i.cv_match DESC,i.analyzed_at DESC LIMIT ?''', params).fetchall()
