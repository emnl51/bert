...
def analyze_job(job_key:str,candidate_profile_id:int,search_job_id:int|None=None)->dict[str,Any]:
    ...
    for term in IMPORTANT:
        if term in job_text:
            (matched if term in cv_text else missing).append(term)
    ...
    risks=[]
    german=_norm(job_text)
    if any(x in german for x in ('c1 german','c2 german','fluent german','business fluent german','verhandlungssicheres deutsch','deutsch c1')): risks.append('German requirement may exceed the current profile')
    if any(x in german for x in ('full-time','full time','vollzeit')) and any('werkstudent' in _norm(x) or 'working student' in _norm(x) for x in c.get('target_roles',[])): risks.append('Employment format may not match the current target')
    if score>=78: rec='apply'
    elif score>=58: rec='maybe'
    else: rec='skip'
    summary=f"{score}/100 CV match. {len(matched)} important requirements are evident; {len(missing)} are not clearly evidenced in the CV."
    with connection() as con:
        con.execute('''INSERT INTO job_intelligence(job_key,candidate_profile_id,search_job_id,cv_match,recommendation,strengths_json,gaps_json,risks_json,matched_terms_json,missing_terms_json,summary,engine,analyzed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(job_key,candidate_profile_id) DO UPDATE SET search_job_id=excluded.search_job_id,cv_match=excluded.cv_match,recommendation=excluded.recommendation,strengths_json=excluded.strengths_json,gaps_json=excluded.gaps_json,risks_json=excluded.risks_json,matched_terms_json=excluded.matched_terms_json,missing_terms_json=excluded.missing_terms_json,summary=excluded.summary,engine=excluded.engine,analyzed_at=excluded.analyzed_at''',(job_key,candidate_profile_id,search_job_id,score,rec,json.dumps(strengths),json.dumps(gaps),json.dumps(risks),json.dumps(matched),json.dumps(missing),summary,'heuristic-v1',_now()))
    return {'job_key':job_key,'candidate_profile_id':candidate_profile_id,'cv_match':score,'recommendation':rec,'strengths':strengths,'gaps':gaps,'risks':risks,'matched_terms':matched,'missing_terms':missing,'summary':summary,'engine':'heuristic-v1'}
