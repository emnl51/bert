...
DEFAULT_KEYWORDS = {
    'search': [
        ('werkstudent supply chain', 0), ('werkstudent supply planning', 0),
        ('werkstudent procurement', 0), ('werkstudent einkauf', 0),
        ('werkstudent order management', 0), ('werkstudent operations', 0),
        ('werkstudent logistik', 0), ('werkstudent material planning', 0),
        ('werkstudent sales operations', 0), ('working student supply chain', 0),
    ],
    'title': [
        ('supply chain', 32), ('supply planning', 32), ('demand planning', 28),
        ('material planning', 28), ('procurement', 30), ('einkauf', 30),
        ('purchasing', 28), ('order management', 28), ('operations', 22),
        ('logistics', 24), ('logistik', 24), ('sales operations', 25),
        ('customer operations', 24), ('strategic sourcing', 25),
    ],
    'format': [
        ('werkstudent', 30), ('working student', 30), ('studentische aushilfe', 25),
        ('student assistant', 25), ('teilzeit', 12), ('part-time', 12),
        ('part time', 12), ('minijob', 8),
    ],
    'skill': [
        ('sap', 7), ('s/4hana', 7), ('power bi', 6), ('excel', 4), ('supplier', 5),
        ('lieferant', 5), ('inventory', 5), ('bestand', 5), ('incoterms', 5),
        ('erp', 5), ('production planning', 7), ('produktionsplanung', 7),
        ('international', 4), ('customer', 4), ('kunde', 4), ('material', 4),
        ('rfq', 5), ('sourcing', 5),
    ],
    'negative': [
        ('software engineer', -30), ('developer', -25), ('nurse', -50),
        ('pflege', -50), ('driver', -35), ('fahrer', -35), ('restaurant', -35),
    ],
}
...
def init_db() -> None:
    ...
    with sqlite3.connect(settings.database_path) as con:
        ...
        if source_count == 0:
            ...
            con.execute(
                'INSERT INTO sources(name,source_type,enabled,config_json,secrets_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?)',
                ('Adzuna', 'adzuna', 0, json.dumps({'distance_km': settings.adzuna_distance_km, 'results_per_term': settings.results_per_term}), json.dumps(adz_secrets), now, now),
            )
...
def get_all_settings(mask_secrets: bool = True) -> dict[str, str]:
    ...
    for row in rows:
        if row['is_secret']:
            result[row['key']] = 'configured' if (mask_secrets and row['value']) else (decrypt_secret(row['value']) if row['value'] else '')
        else:
...
def get_source(source_id: int, mask_secrets: bool = False) -> dict[str, Any] | None:
    ...
    item['config'] = json.loads(item.pop('config_json') or '{}')
    encrypted = json.loads(item.pop('secrets_json') or '{}')
    item['secrets'] = ({k: ('configured' if v else '') for k, v in encrypted.items()} if mask_secrets
                       else {k: decrypt_secret(v) for k, v in encrypted.items()})
    return item


def save_source(name: str, source_type: str, enabled: bool, config: dict, secrets: dict, source_id: int | None = None) -> int:
    now = _now()
    with connection() as con:
        if source_id:
...
def save_keyword(term: str, kind: str, weight: int, enabled: bool, keyword_id: int | None = None) -> int:
    with connection() as con:
        if keyword_id:
            con.execute('UPDATE keywords SET term=?,kind=?,weight=?,enabled=? WHERE id=?', (term.strip().lower(), kind, weight, int(enabled), keyword_id))
            return keyword_id
        cur = con.execute('INSERT INTO keywords(term,kind,weight,enabled) VALUES(?,?,?,?)', (term.strip().lower(), kind, weight, int(enabled)))
        return int(cur.lastrowid)
    ...
    with connection() as con:
        ...
        if existing:
            con.execute(
                ...
                (job.title, job.company, job.location, job.url, job.description, job.created_at,
                 int(job.remote), job.score, json.dumps(job.reasons, ensure_ascii=False), now, job.key),
            )
            return False
        con.execute(
            '''INSERT INTO jobs(job_key, source, external_id, title, company, location, url,
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)''',
            (job.key, job.source, job.external_id, job.title, job.company, job.location, job.url,
             job.description, job.created_at, int(job.remote), job.score,
             json.dumps(job.reasons, ensure_ascii=False), now, now),
        )
        return True
...
def save_application(job_key: str, status: str, notes: str | None = None, applied_at: str | None = None) -> dict[str, Any]:
    ...
    return {'job_key': job_key, 'status': status, 'applied_at': final_applied_at, 'notes': final_notes, 'updated_at': now}
...
def finish_run(run_id: int, *, status: str, fetched: int = 0, new_matches: int = 0,
               provider_errors: list[str] | None = None, notification_channels: list[str] | None = None,
               error: str = '') -> None:
    with connection() as con:
        con.execute(
            ...
            (_now(), status, fetched, new_matches, json.dumps(provider_errors or []),
             json.dumps(notification_channels or []), error, run_id),
        )
...
def dashboard_stats() -> dict[str, int]:
    ...
    return {
        'jobs': jobs, 'notified': notified, 'reviewed': reviewed, 'enabled_sources': enabled_sources,
        'to_apply': apps['to_apply'], 'applications_active': apps['active'],
        'interviews': apps['interview'], 'offers': apps['offer'],
    }
