    ...
    for _ in range(9):
        ...
        if hasattr(node, 'find_all'):
            job_links = [
                ...
                if _JOB_PATH in str(x.get('href') or '')
            ]
        ...
        for term_index, term in enumerate(terms):
            ...
            for page in range(1, pages_per_term + 1):
                ...
                if response.status_code in {403, 429}:
                    raise RuntimeError(f'StepStone blocked the request with HTTP {response.status_code}; source left experimental')
                response.raise_for_status()
                parsed = parse_stepstone_search_html(response.text, source.get('name') or 'StepStone Germany')
                if not parsed:
                    if page == 1:
                        raise RuntimeError('StepStone returned no parseable job cards; page layout or rendering may have changed')
                    break
                for job in parsed:
                    if job.url in seen:
                        continue
