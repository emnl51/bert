(() => {
  ...
  const NAVIGATION = [
    ...
    {key:'settings',label:'Settings',icon:'⚙',primary:'profiles',children:['interface','sources','search','keywords','notifications'],tabsOnly:true},
    {key:'administration',label:'Administration',icon:'▦',primary:'runs',children:['users','systemEmail','database','logs','updates'],tabsOnly:true},
  ];
  const DESTINATIONS = {
  ...
  const CONTEXT_TABS = {
    settings:['profiles','interface','sources','search','keywords','notifications'],
    administration:['runs','users','systemEmail','database','logs','updates']
  };
  const PAGE_CONTEXT = {
  ...
  const TURKISH = {
    ...
    'Match tier':'Eşleşme seviyesi','All relevant roles':'Tüm ilgili roller','Strong':'Güçlü','Match':'Eşleşme','Constraint to review':'Kısıtı incele',
    ...
    'Strong role match':'Güçlü rol eşleşmesi','Relevant role · constraint to review':'İlgili rol · kısıtı inceleyin',
    'strong':'güçlü','match':'eşleşme','stretch':'kısıtlı eşleşme',
    ...
    'One phrase per line. Bert runs unqualified bilingual role queries before schedule-specific variants so capped providers cover every role.':'Her satıra bir ifade. Bert, sınırlı sorgu kabul eden kaynakların her rolü tarayabilmesi için önce çalışma saati eklenmemiş iki dilli rol sorgularını çalıştırır.',
    ...
    'Profile essentials':'Temel profil ayarları','Choose the target level, roles, working arrangements and language ability. Saving also adds the generated search suggestions without removing custom terms.':'Hedef seviyeyi, rolleri, çalışma biçimlerini ve dil yeteneğini seçin. Kaydetme işlemi özel terimleri silmeden oluşturulan arama önerilerini ekler.',
    'Advanced matching settings':'Gelişmiş eşleştirme ayarları','Profile name':'Profil adı','Target role level':'Hedef rol seviyesi','Any suitable level':'Uygun herhangi bir seviye','Technician / inspection staff':'Teknisyen / kalite kontrol personeli','Engineer':'Mühendis','Working student':'Werkstudent',
    ...
    '4. Preview provider-ready search phrases':'4. İlan sitelerine hazır arama ifadelerini önizleyin','Add suggestions now':'Önerileri şimdi ekle','Check profile targeting':'Profil hedeflerini kontrol et',
    ...
    'Student level requires the Working student arrangement.':'Öğrenci seviyesi için Werkstudent çalışma biçimini seçin.','Quality engineering conflicts with technician level.':'Kalite mühendisliği teknisyen seviyesiyle çelişiyor.','Quality technician conflicts with engineering level.':'Kalite teknisyeni mühendislik seviyesiyle çelişiyor.',
    ...
    'Working-time handling':'Çalışma saati yaklaşımı','Prefer profile hours; keep stretch roles':'Profil saatlerini tercih et; kısıtlı rolleri göster','Strictly exclude other/unknown hours':'Diğer veya belirsiz saatleri kesin olarak dışla',
    'Preference mode keeps strong role matches visible when hours differ or are missing.':'Tercih modu, çalışma saatleri farklı veya belirsiz olsa da güçlü rol eşleşmelerini görünür tutar.',
  ...
  function installStyles(){
    ...
    const style=document.createElement('style'); style.id='jobtrack-v16-style'; style.textContent=`
      :root{--jt-sidebar:220px;--jt-sidebar-mini:60px;--jt-mobile-top:64px;--jt-radius:8px;--jt-bg:#f1f0ee;--jt-card:#fbfbfa;--jt-border:#e2e0dc;--jt-ink:#1c1b1a;--jt-soft:#63605d;--jt-blue:#315eea;--jt-blue-dark:#2448c4}
      html{scroll-behavior:smooth}body{background:var(--bg,#f1f0ee);overflow-x:hidden}
      ...
      .side{width:auto;min-width:0;height:100dvh;position:sticky;top:0;padding:0 8px 10px;background:#161616;z-index:40;display:flex;flex-direction:column;overflow:hidden;border-right:1px solid #2a2a2a}
      .brand{padding:20px 8px 16px;font-size:19px;white-space:nowrap;overflow:hidden}.brand small{opacity:.58}.jt-brand-row{display:flex;align-items:center;justify-content:space-between;gap:8px}.jt-collapse{width:34px;height:34px;border:0;background:transparent;color:#94a3b8;border-radius:4px;cursor:pointer;flex:none;font-size:17px}.jt-collapse:hover{background:rgba(255,255,255,.06);color:#fff}
      .nav{display:flex;flex:1;flex-direction:column;gap:2px;overflow-y:auto;overflow-x:hidden;padding:0 1px 12px;scrollbar-width:thin}
      .bert-nav-section{display:grid;gap:3px}.bert-nav-row{display:grid;grid-template-columns:minmax(0,1fr) 34px;gap:3px}
      .bert-nav-button,.bert-nav-toggle{display:flex;align-items:center;gap:11px;width:100%;min-height:38px;margin:0;padding:7px 10px;border:1px solid transparent;border-radius:4px;background:transparent;color:#aeb9ca;font-size:13px;font-weight:650;text-align:left;white-space:nowrap;cursor:pointer;transition:background .14s ease,color .14s ease}.bert-nav-button:hover,.bert-nav-toggle:hover{background:rgba(255,255,255,.06);color:#efefef;transform:none}.bert-nav-button.active{background:rgba(79,111,219,.12);border-color:rgba(79,111,219,.25);color:#efefef;box-shadow:none}.bert-nav-section.has-active>.bert-nav-row .bert-nav-button,.bert-nav-section.has-active>.bert-nav-toggle{color:#f8fafc}
      .bert-nav-icon{display:grid;place-items:center;width:22px;min-width:22px;height:22px;color:#8998ae;font-size:14px;line-height:1}.bert-nav-button.active .bert-nav-icon{color:#c7d3ff}.bert-nav-label{min-width:0;overflow:hidden;text-overflow:ellipsis}.bert-nav-expander{display:grid;place-items:center;width:32px;min-height:38px;padding:0;border:0;border-radius:4px;background:transparent;color:#7f8da3;cursor:pointer}.bert-nav-expander:hover{background:rgba(255,255,255,.07);color:#fff}.bert-nav-chevron{transition:transform .16s ease}.bert-nav-section.is-open .bert-nav-chevron{transform:rotate(90deg)}
      .bert-nav-submenu{display:none;gap:2px;margin:1px 0 5px 33px;padding:3px 0 3px 10px;border-left:1px solid rgba(148,163,184,.18)}.bert-nav-section.is-open>.bert-nav-submenu{display:grid}.bert-nav-submenu .bert-nav-button{min-height:34px;padding:6px 9px;font-size:12px;font-weight:540}.bert-nav-submenu .bert-nav-icon{width:18px;min-width:18px;height:18px;font-size:11px}
      .app.jt-collapsed .brand .jt-brand-text,.app.jt-collapsed .brand small,.app.jt-collapsed .bert-nav-label{display:none}.app.jt-collapsed .brand{padding-inline:0}.app.jt-collapsed .jt-brand-row{justify-content:center}.app.jt-collapsed .bert-nav-row{display:block}.app.jt-collapsed .bert-nav-button,.app.jt-collapsed .bert-nav-toggle{justify-content:center;width:44px;height:38px;margin-inline:auto;padding:0}.app.jt-collapsed .bert-nav-icon{width:24px;min-width:24px;height:24px;font-size:17px}.app.jt-collapsed .bert-nav-expander,.app.jt-collapsed .bert-nav-submenu{display:none}.bert-nav-route-store{display:none!important}.app.jt-collapsed .jt-collapse{transform:rotate(180deg)}
      .bert-context-tabs{display:flex;gap:4px;margin:-4px 0 20px;padding:0 0 8px;border-bottom:1px solid var(--line);overflow-x:auto;scrollbar-width:thin}.bert-context-tabs[hidden]{display:none}.bert-context-tabs button{flex:none;min-height:34px;padding:6px 10px;border:0;border-radius:4px;background:transparent;color:var(--muted);font:inherit;font-size:12px;font-weight:600;cursor:pointer}.bert-context-tabs button:hover{background:var(--surface-soft);color:var(--text)}.bert-context-tabs button.active{background:color-mix(in srgb,var(--accent-solid) 10%,transparent);color:var(--text);box-shadow:inset 0 -2px 0 var(--accent-solid)}
      .main{max-width:none;width:100%;min-width:0;padding:24px 24px 50px}.top{min-height:62px;position:sticky;top:0;z-index:20;margin:-10px 0 20px;padding:10px 0;background:rgba(248,250,252,.86);backdrop-filter:blur(14px);display:flex;gap:14px}.top h1{font-size:clamp(21px,2vw,27px);letter-spacing:-.035em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.top-actions{display:flex;gap:8px;align-items:center}.jt-mobile-menu{display:none;width:42px;height:42px;border:1px solid var(--jt-border);background:white;border-radius:10px;font-size:20px;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 3px 10px rgba(15,23,42,.05)}
      .card{border-radius:var(--jt-radius);border-color:var(--jt-border);box-shadow:var(--shadow);padding:clamp(14px,1.8vw,20px)}.grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:13px}.metric{font-size:clamp(23px,2.2vw,30px)}.section-title{margin:26px 0 11px}.section-title h2{font-size:19px;letter-spacing:-.015em}
      ...
      :root{--jt-radius:8px;--jt-bg:#f1f0ee;--jt-blue:#315eea;--jt-blue-dark:#2448c4}
      body{background:var(--bg)}.side{border-right:1px solid var(--line,#2a2a2a)}
      ...
      :root{color-scheme:light;--bg:#f1f0ee;--card:#fbfbfa;--text:#1c1b1a;--muted:#63605d;--line:#e2e0dc;--accent:#315eea;--accent-solid:#315eea;--accent2:#e8ecf8;--on-accent:#ffffff;--surface-soft:#ebe9e6;--focus:#315eea;--good:#2f7d5d;--goodbg:#dcece4;--bad:#b03a4a;--badbg:#f5dde1;--warn:#9a6b12;--warnbg:#f5ead1;--shadow:0 2px 8px rgba(28,27,26,.10);--jt-bg:var(--bg);--jt-card:var(--card);--jt-border:var(--line);--jt-ink:var(--text);--jt-soft:var(--muted);--jt-blue:var(--accent-solid);--jt-blue-dark:#2448c4}
      html[data-theme="dark"]{color-scheme:dark;--bg:#0d0d0d;--card:#161616;--text:#efefef;--muted:#909090;--line:#2a2a2a;--accent:#7f9be8;--accent-solid:#4f6fdb;--accent2:#202536;--on-accent:#ffffff;--surface-soft:#1e1e1e;--focus:#7f9be8;--good:#6cb597;--goodbg:#153b2a;--bad:#d4707c;--badbg:#462329;--warn:#d8a34a;--warnbg:#41331a;--shadow:0 2px 8px rgba(0,0,0,.45);--jt-blue:var(--accent-solid);--jt-blue-dark:#3f5fc4}
  ...
  function buildNavigation(){
    ...
    for(const data of NAVIGATION){const primary=data.primary?sourceButtons.get(data.primary):null;const children=data.children.map(tab=>sourceButtons.get(tab)).filter(Boolean);if(!primary&&!children.length)continue;
      ...
      if(primary){primary.className='bert-nav-button';primary.innerHTML=data.tabsOnly?`<span class="bert-nav-icon" aria-hidden="true">${data.icon}</span><span class="bert-nav-label">${localizedLabel(data.label)}</span>`:destinationMarkup(primary.dataset.tab);primary.title=localizedLabel(data.label);const row=document.createElement('div');row.className='bert-nav-row';row.appendChild(primary);
      ...
      if(children.length){submenu.classList.toggle('bert-nav-route-store',Boolean(data.tabsOnly));section.appendChild(submenu)}const containsActive=primary===active||children.includes(active);section.classList.toggle('has-active',containsActive);setSectionExpanded(section,containsActive);nav.appendChild(section)
    }
  }
  ...
  function installContextTabs(main){
    if($('bertContextTabs'))return;
    const bar=document.createElement('nav');bar.id='bertContextTabs';bar.className='bert-context-tabs';bar.setAttribute('aria-label','Section pages');
    for(const [sectionKey,tabs] of Object.entries(CONTEXT_TABS)){for(const tab of tabs){const button=document.createElement('button');button.type='button';button.dataset.contextTab=tab;button.dataset.contextSection=sectionKey;button.textContent=localizedLabel(DESTINATIONS[tab]?.label||tab);button.addEventListener('click',()=>document.querySelector(`.nav button[data-tab="${tab}"]`)?.click());bar.appendChild(button)}}
    main.querySelector('.top')?.after(bar);
  }
  function updateContextTabs(tab){
    const bar=$('bertContextTabs');if(!bar)return;const section=NAVIGATION.find(item=>item.primary===tab||item.children.includes(tab));const visible=Boolean(section?.tabsOnly);bar.hidden=!visible;
    bar.querySelectorAll('button').forEach(button=>{button.hidden=!visible||button.dataset.contextSection!==section?.key;button.classList.toggle('active',button.dataset.contextTab===tab);button.setAttribute('aria-current',button.dataset.contextTab===tab?'page':'false')});
  }
  ...
  function installShell(){
    ...
    installInterfaceSettings(side.querySelector('.nav'),main);buildNavigation();installContextTabs(main);installNavigationSearch(side);annotateTables();installOverviewShortcuts();
    ...
    const initialTab=document.querySelector('.nav button[data-tab].active')?.dataset.tab||'overview';updatePageContext(initialTab);updateContextTabs(initialTab);
    side.addEventListener('click',e=>{const btn=e.target.closest('.nav button[data-tab]');if(!btn)return;revealSection(btn.dataset.tab);updatePageContext(btn.dataset.tab);updateContextTabs(btn.dataset.tab);try{localStorage.setItem('jobtrack-tab',btn.dataset.tab)}catch(_){}setTimeout(()=>{if($('pageTitle'))$('pageTitle').textContent=localizedLabel(DESTINATIONS[btn.dataset.tab]?.label||btn.textContent.trim())},0)});
