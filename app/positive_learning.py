...
'''
EVENT_STRENGTH={'suitable':4,'applied':5,'interview':9,'offer':14}; EVENT_RANK={'suitable':1,'applied':2,'interview':3,'offer':4}
STOPWORDS={'werkstudent','working','student','studentin','studentische','m','w','d','f','x','and','und','the','for','für','im','in','of','at','bei','part','time','teilzeit','intern','internship','praktikum','praktikant','praktikantin','junior','senior','manager'}
SKILL_TERMS=('supply chain','supply planning','demand planning','material planning','procurement','einkauf','purchasing','order management','operations','logistics','logistik','strategic sourcing','sourcing','supplier','lieferant','sap','s/4hana','power bi','excel','erp','inventory','bestand','production planning','produktionsplanung','international','customer','kunde','material','rfq','incoterms')
...
def ensure_positive_schema():
    ...
    with connection() as con:
        ...
        if 'profile_id' not in cols: con.execute('ALTER TABLE positive_rules ADD COLUMN profile_id INTEGER NOT NULL DEFAULT 1')
        cols={r[1] for r in con.execute('PRAGMA table_info(positive_events)').fetchall()}
        if 'profile_id' not in cols: con.execute('ALTER TABLE positive_events ADD COLUMN profile_id INTEGER NOT NULL DEFAULT 1')
        ...
        if ddl and 'UNIQUE(scope, term)' in (ddl['sql'] or ''):
            ...
            con.execute('''CREATE TABLE positive_rules (id INTEGER PRIMARY KEY AUTOINCREMENT,profile_id INTEGER NOT NULL DEFAULT 1,scope TEXT NOT NULL,term TEXT NOT NULL,weight INTEGER NOT NULL DEFAULT 4,evidence_count INTEGER NOT NULL DEFAULT 1,enabled INTEGER NOT NULL DEFAULT 1,strongest_event TEXT NOT NULL DEFAULT 'suitable',created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(profile_id,scope,term))''')
            con.execute('''INSERT INTO positive_rules(id,profile_id,scope,term,weight,evidence_count,enabled,strongest_event,created_at,updated_at) SELECT id,COALESCE(profile_id,1),scope,term,weight,evidence_count,enabled,strongest_event,created_at,updated_at FROM positive_rules_old_v9''')
        ...
        if ddl and 'UNIQUE(job_key, event_type)' in (ddl['sql'] or ''):
            ...
            con.execute('''CREATE TABLE positive_events (id INTEGER PRIMARY KEY AUTOINCREMENT,job_key TEXT NOT NULL,profile_id INTEGER NOT NULL DEFAULT 1,event_type TEXT NOT NULL,created_at TEXT NOT NULL,UNIQUE(job_key,profile_id,event_type),FOREIGN KEY(job_key) REFERENCES jobs(job_key) ON DELETE CASCADE)''')
            con.execute('''INSERT INTO positive_events(id,job_key,profile_id,event_type,created_at) SELECT id,job_key,COALESCE(profile_id,1),event_type,created_at FROM positive_events_old_v9''')
            con.execute('DROP TABLE positive_events_old_v9')
        con.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_positive_rule_profile_scope_term ON positive_rules(profile_id,scope,term)')
        con.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_positive_event_profile ON positive_events(job_key,profile_id,event_type)')
...
def _extract_positive_rules(job,language_label,event_type):
    ...
    if event_type in ('interview','offer') and _normalise(job.get('company','')): rules.append({'scope':'company','term':_normalise(job.get('company','')),'weight':base})
    return rules[:8]

def _upsert(con,profile_id,rule,event_type):
    row=con.execute('SELECT id,evidence_count,weight,strongest_event FROM positive_rules WHERE profile_id=? AND scope=? AND term=?',(profile_id,rule['scope'],rule['term'])).fetchone(); now=_now()
    if row:
        ev=int(row['evidence_count'])+1; strongest=row['strongest_event'] if EVENT_RANK.get(row['strongest_event'],0)>=EVENT_RANK[event_type] else event_type
        target=max(int(row['weight']),int(rule['weight']))+(1 if ev in (2,4,7,12) else 0); weight=min(30,target)
        con.execute('UPDATE positive_rules SET evidence_count=?,weight=?,enabled=1,strongest_event=?,updated_at=? WHERE id=?',(ev,weight,strongest,now,row['id'])); return int(row['id'])
    cur=con.execute('INSERT INTO positive_rules(profile_id,scope,term,weight,evidence_count,enabled,strongest_event,created_at,updated_at) VALUES(?,?,?,?,1,1,?,?,?)',(profile_id,rule['scope'],rule['term'],int(rule['weight']),event_type,now,now)); return int(cur.lastrowid)

def record_positive_event(job_key,event_type,profile_id=1):
    if event_type not in EVENT_STRENGTH: raise ValueError('Invalid positive event')
...
def list_positive_rules(profile_id=None):
    ensure_positive_schema(); where=''; params=[]
    if profile_id: where='WHERE profile_id=?'; params=[profile_id]
    with connection() as con: rows=con.execute(f'SELECT * FROM positive_rules {where} ORDER BY enabled DESC,evidence_count DESC,weight DESC,term',params).fetchall()
    return [{**dict(r),'enabled':bool(r['enabled'])} for r in rows]
def set_positive_rule_enabled(rule_id,enabled):
    with connection() as con: con.execute('UPDATE positive_rules SET enabled=?,updated_at=? WHERE id=?',(int(enabled),_now(),rule_id))
def delete_positive_rule(rule_id):
    with connection() as con: con.execute('DELETE FROM positive_rules WHERE id=?',(rule_id,))

def apply_positive_boost(job,base_score,profile_id=1):
    ensure_positive_schema()
    with connection() as con: rules=con.execute('SELECT scope,term,weight FROM positive_rules WHERE enabled=1 AND profile_id=?',(profile_id,)).fetchall()
    ...
    boost=0; reasons=[]
    for r in rules:
        if r['term'] and r['term'] in fields.get(r['scope'],''): boost+=int(r['weight']); reasons.append(f"preferred: {r['scope']} '{r['term']}' +{r['weight']}")
    boost=min(30,boost); return min(100,int(base_score)+boost),reasons

def positive_stats(profile_id=None):
    ensure_positive_schema(); params=[]; pf=''
    ...
    with connection() as con:
        ...
        rules=con.execute('SELECT COUNT(*) FROM positive_rules'+(pf+' AND ' if pf else ' WHERE ')+'enabled=1',params).fetchone()[0]
        interviews=con.execute('SELECT COUNT(*) FROM positive_events'+(pf+' AND ' if pf else ' WHERE ')+"event_type='interview'",params).fetchone()[0]
        offers=con.execute('SELECT COUNT(*) FROM positive_events'+(pf+' AND ' if pf else ' WHERE ')+"event_type='offer'",params).fetchone()[0]
