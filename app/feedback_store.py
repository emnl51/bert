...
def ensure_feedback_schema():
    with connection() as con:
        ...
        if "profile_id" not in cols:
            con.execute("ALTER TABLE job_feedback ADD COLUMN profile_id INTEGER NOT NULL DEFAULT 1")
        cols = {r[1] for r in con.execute("PRAGMA table_info(learned_rules)").fetchall()}
        if "profile_id" not in cols:
            con.execute("ALTER TABLE learned_rules ADD COLUMN profile_id INTEGER NOT NULL DEFAULT 1")
        ...
        if ddl and "UNIQUE(scope, term)" in (ddl["sql"] or ""):
            ...
            con.execute(
                """CREATE TABLE learned_rules (id INTEGER PRIMARY KEY AUTOINCREMENT,profile_id INTEGER NOT NULL DEFAULT 1,scope TEXT NOT NULL,term TEXT NOT NULL,weight INTEGER NOT NULL DEFAULT -8,evidence_count INTEGER NOT NULL DEFAULT 1,enabled INTEGER NOT NULL DEFAULT 1,source_reason TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(profile_id,scope,term))"""
            )
            con.execute(
                """INSERT INTO learned_rules(id,profile_id,scope,term,weight,evidence_count,enabled,source_reason,created_at,updated_at) SELECT id,COALESCE(profile_id,1),scope,term,weight,evidence_count,enabled,source_reason,created_at,updated_at FROM learned_rules_old_v9"""
            )
            con.execute("DROP TABLE learned_rules_old_v9")
        con.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_learned_profile_scope_term ON learned_rules(profile_id,scope,term)"
        )
...
def _suggest_rules(job, reason):
    ...
    if reason == "wrong_role":
        rules += [{"scope": "title", "term": t, "weight": -8} for t in _role_terms(job.get("title", ""))]
    elif reason == "company" and _normalise(job.get("company", "")):
        rules = [{"scope": "company", "term": _normalise(job.get("company", "")), "weight": -10}]
    elif reason == "location" and _normalise(job.get("location", "")):
        rules = [{"scope": "location", "term": _normalise(job.get("location", "")), "weight": -6}]
    elif reason == "seniority":
        title = _normalise(job.get("title", ""))
        rules = [
            {"scope": "title", "term": t, "weight": -7}
            for t in ("senior", "lead", "head", "director", "manager", "junior", "intern", "praktikum")
            if t in title
        ]
    elif reason == "employment_type":
        text = _normalise(f"{job.get('title', '')} {job.get('description', '')}")
        rules = [
            {"scope": "description", "term": t, "weight": -7}
            for t in ("full-time", "full time", "vollzeit", "internship", "praktikum", "minijob")
            if t in text
        ]
    elif reason == "german_level":
        rules = [{"scope": "language", "term": "german_heavy", "weight": -12}]
    return rules[:3]


def _upsert_rule(con, profile_id, rule, reason):
    row = con.execute(
        "SELECT id,evidence_count,weight FROM learned_rules WHERE profile_id=? AND scope=? AND term=?",
        (profile_id, rule["scope"], rule["term"]),
    ).fetchone()
    now = _now()
    if row:
        ev = int(row["evidence_count"]) + 1
        weight = max(-30, min(int(row["weight"]), int(rule["weight"])) - (1 if ev in (2, 4, 7) else 0))
        con.execute(
            "UPDATE learned_rules SET evidence_count=?,weight=?,enabled=1,source_reason=?,updated_at=? WHERE id=?",
            (ev, weight, reason, now, row["id"]),
        )
        return int(row["id"])
    cur = con.execute(
        "INSERT INTO learned_rules(profile_id,scope,term,weight,evidence_count,enabled,source_reason,created_at,updated_at) VALUES(?,?,?,?,1,1,?,?,?)",
        (profile_id, rule["scope"], rule["term"], int(rule["weight"]), reason, now, now),
    )
...
def list_feedback(limit=100, profile_id=None):
    ...
    params = []
    if profile_id:
        where = "WHERE f.profile_id=?"
        params.append(profile_id)
    params.append(limit)
    with connection() as con:
        rows = con.execute(
            f"""SELECT f.*,j.title,j.company,j.location FROM job_feedback f JOIN jobs j ON j.job_key=f.job_key {where} ORDER BY f.id DESC LIMIT ?""",
            params,
        ).fetchall()
...
def list_learned_rules(profile_id=None):
    ...
    return sorted(
        positive + negative, key=lambda r: (not r["enabled"], -r["evidence_count"], -abs(r["weight"]), r["term"])
    )
...
def apply_learned_penalty(job, base_score, profile_id=1):
    ...
    for r in rules:
        if r["term"] and r["term"] in fields.get(r["scope"], ""):
            score += int(r["weight"])
            reasons.append(f"learned penalty: {r['scope']} '{r['term']}' {r['weight']}")
...
def feedback_stats(profile_id=None):
    ...
    with connection() as con:
        ...
        bad = con.execute(
            "SELECT COUNT(*) FROM job_feedback" + (pf + " AND " if pf else " WHERE ") + "suitability='not_suitable'",
            params,
        ).fetchone()[0]
        rules = con.execute(
