(() => {
  ...
  window.addCatalogSource=function(key){
    ...
    if(c.source_type==='kleinanzeigen') return showModal(c.name,c.description,[{id:'name',label:'Display name',value:'Kleinanzeigen Jobs'},{id:'location',label:'Location',value:'Berlin',hint:'The visible city name used in Kleinanzeigen URLs.'},{id:'location_id',label:'Kleinanzeigen location ID',value:'3331',hint:'Berlin is 3331. Copy another city ID from an exact Kleinanzeigen search URL.'},{id:'arrangement',label:'Working arrangement',type:'select',value:'both',options:[{value:'both',label:'Full-time and part-time'},{value:'full_time',label:'Full-time only'},{value:'part_time',label:'Part-time only'}],hint:'Adds Vollzeit or Teilzeit to provider queries when one format is selected.'},{id:'age',label:'Listing age',type:'select',value:'30',options:[{value:'0',label:'Any date'},{value:'7',label:'Last 7 days'},{value:'30',label:'Last 30 days'},{value:'90',label:'Last 90 days'}],hint:'Older listings are excluded when their date can be determined.'},{id:'radius',label:'Radius (km)',value:'40',hint:'0–200 km'},{id:'terms',label:'Max profile search phrases',value:'6',hint:'1–20; queries keep their profile order.'},{id:'pages',label:'Pages per phrase',value:'1',hint:'1–5'},{id:'details',label:'Detail pages per run',value:'10',hint:'Fetches complete descriptions, work type, address and date for analysis.'}],async v=>{await api('/api/sources',{method:'POST',body:JSON.stringify({name:v.name||c.name,source_type:'kleinanzeigen',enabled:true,config:{location_name:v.location.trim(),location_id:v.location_id.trim(),working_arrangement:v.arrangement,max_age_days:Math.max(0,Math.min(365,+v.age||0)),radius_km:Math.max(0,Math.min(200,+v.radius||0)),max_search_terms:Math.max(1,Math.min(20,+v.terms||6)),pages_per_term:Math.max(1,Math.min(5,+v.pages||1)),detail_limit:Math.max(0,Math.min(50,+v.details||0)),request_delay_seconds:1},secrets:{}})});toast('Kleinanzeigen configured');await loadSources()});
  ...
  window.editSource=async function(id){
    ...
    if(s.source_type==='kleinanzeigen')return showModal('Kleinanzeigen settings','Fetch complete ad content while keeping requests conservative.',[{id:'name',label:'Display name',value:s.name},{id:'location',label:'Location',value:config.location_name||'Berlin'},{id:'location_id',label:'Kleinanzeigen location ID',value:config.location_id||''},{id:'arrangement',label:'Working arrangement',type:'select',value:config.working_arrangement||'both',options:[{value:'both',label:'Full-time and part-time'},{value:'full_time',label:'Full-time only'},{value:'part_time',label:'Part-time only'}]},{id:'age',label:'Listing age',type:'select',value:String(config.max_age_days??30),options:[{value:'0',label:'Any date'},{value:'7',label:'Last 7 days'},{value:'30',label:'Last 30 days'},{value:'90',label:'Last 90 days'}]},{id:'radius',label:'Radius (km)',value:String(config.radius_km??40)},{id:'terms',label:'Max profile search phrases',value:String(config.max_search_terms||6)},{id:'pages',label:'Pages per phrase',value:String(config.pages_per_term||1)},{id:'details',label:'Detail pages per run',value:String(config.detail_limit??10),hint:'Complete descriptions feed language, work-type and card analysis.'}],async v=>{s.name=v.name||s.name;config.location_name=v.location.trim();config.location_id=v.location_id.trim();config.working_arrangement=v.arrangement;config.max_age_days=Math.max(0,Math.min(365,+v.age||0));config.radius_km=Math.max(0,Math.min(200,+v.radius||0));config.max_search_terms=Math.max(1,Math.min(20,+v.terms||6));config.pages_per_term=Math.max(1,Math.min(5,+v.pages||1));config.detail_limit=Math.max(0,Math.min(50,+v.details||0));config.request_delay_seconds=1;await saveExisting(s,config,{})});
