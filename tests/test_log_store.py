...
def test_log_redacts_secrets_and_jooble_key():
    text = log_store.redact_log_text("token=abc123 https://jooble.org/api/super-secret-key authorization: Bearer xyz")
...
def test_log_filters_by_level_logger_and_query():
    ...
    handler.setFormatter(logging.Formatter("%(message)s"))
    handler.emit(
        logging.LogRecord("jobtrack.provider", logging.INFO, __file__, 1, "Arbeitnow fetched 100 jobs", (), None)
    )
