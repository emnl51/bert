...
def test_engineer_title_alone_does_not_create_degree_requirement(tmp_path, monkeypatch):
    ...
    job = Job(
        ...
        description="Automotive manufacturing role with PFMEA and SPC responsibilities.",
    )
    db.upsert_job(job)
                ...
                "response": json.dumps(
                    {
                        ...
                        "context_notes": [
                            {
                                "evidence_ref": "tools:sap",
                                "text": "SAP is a gap; ERP experience is adjacent but not proof of SAP.",
                            }
                        ],
                        "transferable": [
                            {"evidence_ref": "tools:sap", "text": "ERP experience may help onboarding to SAP."}
                        ],
        ...
        def json(self):
            return {
                "response": json.dumps(
                    {
                        "contextual_score": 95,
                        "context_notes": [{"evidence_ref": "invented:skill", "text": "Invented skill"}],
                        "transferable": [],
                    }
                )
            }
