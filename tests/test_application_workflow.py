...
def setup_db(tmp_path, monkeypatch):
    ...
    db.init_db()
    job = Job(
        source='Test', external_id='abc123', title='Werkstudent Supply Chain',
        company='Example GmbH', location='Berlin', url='https://example.com/job', score=80,
    )
...
def test_rescan_preserves_review_decision(tmp_path, monkeypatch):
    ...
    rescanned = Job(
        source='Test', external_id='abc123', title='Werkstudent Supply Chain - Updated',
        company='Example GmbH', location='Berlin', url='https://example.com/job', score=92,
    )
