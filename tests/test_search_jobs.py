...
def setup_db(tmp_path, monkeypatch):
    ...
    db.init_db(); ensure_profile_schema(); ensure_search_job_schema()


def test_multiple_search_jobs_can_share_profile(tmp_path, monkeypatch):
    setup_db(tmp_path, monkeypatch)
    p=list_profiles()[0]
    one=save_search_job({'name':'Berlin Daily','profile_id':p['id'],'target_location':'Berlin','frequency':'daily'})
    two=save_search_job({'name':'Hennigsdorf Weekly','profile_id':p['id'],'target_location':'Hennigsdorf','frequency':'weekly'})
...
def test_search_job_notification_settings_are_isolated(tmp_path, monkeypatch):
    setup_db(tmp_path, monkeypatch)
    p=list_profiles()[0]
    a=save_search_job({'name':'Telegram Search','profile_id':p['id'],'notify_telegram':True,'notification':{'telegram_chat_id':'123'}})
    b=save_search_job({'name':'Email Search','profile_id':p['id'],'notify_email':True,'notification':{'email_to':'user@example.com'}})
