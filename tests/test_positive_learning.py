...
from app.positive_learning import (
    ensure_positive_schema, record_positive_event, list_positive_rules,
    apply_positive_boost, sync_application_events,
)
...
def add_job():
    job = Job(
        source='test', external_id='1', title='Working Student Supply Chain Procurement',
        company='Example GmbH', location='Berlin', url='https://example.com/job',
...
def test_suitable_event_is_idempotent_and_boosts_similar_jobs(tmp_path, monkeypatch):
    ...
    assert rules
    candidate = Job(
        source='test', external_id='2', title='Working Student Supply Chain', company='Other GmbH',
        location='Berlin', url='https://example.com/2', description='Supply chain role with SAP and Excel.'
    )
...
def test_application_status_sync_is_cumulative(tmp_path, monkeypatch):
    ...
    with db.connection() as con:
        events = {r['event_type'] for r in con.execute('SELECT event_type FROM positive_events WHERE job_key=?', (job.key,)).fetchall()}
